...
static int tegra_bo_iommu_map(struct tegra_drm *tegra, struct tegra_bo *bo)
{
	...
	bo->iosize = iommu_map_sg(tegra->domain, bo->dmaaddr, bo->sgt->sgl,
				  bo->sgt->nents, prot);
	if (!bo->iosize) {
...
static int tegra_bo_iommu_unmap(struct tegra_drm *tegra, struct tegra_bo *bo)
{
	...
	if (IS_ENABLED(CONFIG_TEGRA_IOMMU_GART) && tegra->has_gart) {
		...
		iommu_unmap(tegra->domain, bo->dmaaddr, bo->iosize);
		drm_mm_remove_node(&bo->mm);
	}
unlock:
	mutex_unlock(&tegra->mm_lock);
					      ...
					      size_t size)
{
	...
	/* memory controller traps these addresses on all Tegra's */
	bo->gartaddr	= 0x666DEAD0;
	bo->dmaaddr	= 0x666DEAD0;
	bo->paddr	= 0x666DEAD0;
