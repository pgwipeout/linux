...
static inline bool tegra_drm_job_progresses(struct tegra_drm_job *job)
{
	struct tegra_drm_channel *drm_channel = job->drm_channel;
	struct host1x_channel *channel = drm_channel->channel;
	dma_addr_t dma_addr;

	dma_addr = host1x_channel_dmaget(channel);

	/*
	 * Get DMAGET address on the first timeout, on the second check if
	 * address changed. Note that seqno is unique value.
	 */
	if (drm_channel->hangcheck_dma_addr != dma_addr ||
	    drm_channel->hangcheck_seqno != job->hw_fence->seqno) {

		drm_channel->hangcheck_dma_addr = dma_addr;
		drm_channel->hangcheck_seqno = job->hw_fence->seqno;
...
static inline void tegra_drm_recover_hardware(struct tegra_drm_job *drm_job)
{
	...
	/*
	 * DMA address is aligned to 4 bytes at least, hence it only
	 * matters to reset hangcheck_dma_addr to a some unaligned value.
	 */
	drm_channel->hangcheck_dma_addr = 0xffffffff;
...
static void tegra_drm_sched_timedout_job(struct drm_sched_job *sched_job)
{
	...
	if (tegra_drm_job_progresses(job))
		return;
